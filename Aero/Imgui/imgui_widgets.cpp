...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(held && hovered) ? ImColor(47, 47, 47) : hovered ? ImColor(47, 47, 47) : ImColor(47, 47, 47), true, style.FrameRounding);
    if (*v)
    {
        const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
        const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad + 0.5f, pad + 0.5f), check_bb.Max - ImVec2(pad + 0.5f, pad + 0.5f), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
    }
...
float RoundScalarAAAAAA(float value, int decimal_precision)
{
...
bool SliderBehaviorAAAAA(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ImGuiContext& g = *GImGui;
    ...
    if (g.ActiveId == id)
    {
        ...
        if (set_new_value)
        {
            ...
            // Round past decimal precision
            new_value = RoundScalarAAAAAA(new_value, decimal_precision);
            if (*v != new_value)
            {
                *v = new_value;
                value_changed = true;
    ...
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 10), ImColor(39, 39, 39), 2);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 5, grab_bb.Min.y + 8), 6, ImColor(148, 33, 255), 24);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(grab_bb.Min.x, frame_bb.Min.y + 10), ImColor(148, 33, 255), 2);
...
static inline void AAAADataTypeFormatString(ImGuiDataType data_type, void* data_ptr, int decimal_precision, char* buf, int buf_size)
{
    if (data_type == ImGuiDataType_Float)
    {
        if (decimal_precision < 0)
...
bool InputScalarAsWidgetReplacementa(const ImRect& aabb, const char* label, ImGuiDataType data_type, void* data_ptr, ImGuiID id, int decimal_precision)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ...
    char buf[32];
    AAAADataTypeFormatString(data_type, data_ptr, decimal_precision, buf, IM_ARRAYSIZE(buf));
    bool text_value_changed = ImGui::InputTextEx(label, buf, IM_ARRAYSIZE(buf), aabb.GetSize(), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_AutoSelectAll);
...
int AAAAAAParseFormatPrecision(const char* fmt, int default_precision)
{
    int precision = default_precision;
    while ((fmt = strchr(fmt, '%')) != NULL)
    {
...
//   "Gold: %.0f"   Gold: 1
bool ImGui::SliderFloaAAAAAAAAAt(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!display_format)
        display_format = "%.3f";
    int decimal_precision = AAAAAAParseFormatPrecision(display_format, 3);
    ...
    if (start_text_input || (g.ActiveId == id && g.ScalarAsInputTextId == id))
        return InputScalarAsWidgetReplacementa(frame_bb, label, ImGuiDataType_Float, v, id, decimal_precision);

    // Actual slider behavior + render grab
    ImGui::ItemSize(total_bb, style.FramePadding.y);
    const bool value_changed = SliderBehaviorAAAAA(frame_bb, id, v, v_min, v_max, power, decimal_precision, ImGuiSliderFlags_None);
